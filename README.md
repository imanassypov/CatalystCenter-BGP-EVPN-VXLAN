...
- **Spine MCLUSTER-LOOPBACKS**: Only each DMZ's `Loopback1` (`dmz1` `10.100.2.1/32`, `dmz2` `10.100.4.1/32`) is redistributed into OSPF 1 via `ip prefix-list MCLUSTER-LOOPBACKS` on spines. This ensures borders have a specific route to each GRE hub. Injecting the DMZ Lo0 (`198.19.1.200` / `198.19.2.200`) into OSPF 1 would decouple `fall-over` detection from tunnel state. The block is preceded by a compliance-ignored `no ip prefix-list MCLUSTER-LOOPBACKS` so the list is rebuilt from scratch on every push — Catalyst Center templates are additive, and IOS-XE will not overwrite an occupied prefix-list sequence number, so a stale day-0 entry would otherwise block the intended prefix permanently. See [Release Notes/2026-08-13-mcluster-prefix-list-rebuild.md](Release%20Notes/2026-08-13-mcluster-prefix-list-rebuild.md).
...
- **Border shared-RT import**: After `rewrite-evpn-rt-asn`, DMZ shared Type-5 (`65003:1000` / `65004:1000`) becomes `65001:1000`. `FABRIC-VRF.j2` imports that RT (and stitching) into border tenant VRFs only — gated on `DEFN_TUNNELS` via `DEFN_VRF_BORDER_IMPORT_RTS`. This is required for bidirectional multi-cluster: without it, dmz2 prefixes are received but `PfxRcd` stays 0. Shared is **not** instantiated as a campus VRF/L3VNI. See [Release Notes/2026-09-01-dmz2-bidirectional-evpn-rt-import.md](Release%20Notes/2026-09-01-dmz2-bidirectional-evpn-rt-import.md).
